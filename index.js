...
//index.html 파일의  <input type="text" id="todoValue">를 아이디명(#todoValue)으로 불러와 어딘가에 존재하게한다.delete
    //타입을 input "text"에서 checkbox로 바꿔줌. 그리고 $(input.vlaue)를 통해 검색어를 받는다.
    for(i in todos){
...
add.addEventListener('click', function(v){
    if(input.value === ""){// =는 할당이니 조심! ===쓰는 습관!! 의미는 만약 value가 ==="" 비어 있다면 
        alert("값을 입력하세요. ")//alert실행되는 목록이다 .
    }else{//만약 if가아니라면 else가 실행됨
    createTodo(input.value)//이함수 안에는 많은 코드가 들어가있다. <ul><li></li><button></button></ul>이 생성되어있고 value를 받아 자기가 원하는 글으 글등록한다
    }
})
